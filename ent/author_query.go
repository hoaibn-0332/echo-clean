...
func (aq *AuthorQuery) QueryArticle() *ArticleQuery {
	query := (&ArticleClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, author.ArticleTable, author.ArticleColumn),
		)
...
func (aq *AuthorQuery) WithArticle(opts ...func(*ArticleQuery)) *AuthorQuery {
	query := (&ArticleClient{config: aq.config}).Query()
...
func (aq *AuthorQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Author, error) {
	...
	if query := aq.withArticle; query != nil {
		if err := aq.loadArticle(ctx, query, nodes,
			func(n *Author) { n.Edges.Article = []*Article{} },
			func(n *Author, e *Article) { n.Edges.Article = append(n.Edges.Article, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AuthorQuery) loadArticle(ctx context.Context, query *ArticleQuery, nodes []*Author, init func(*Author), assign func(*Author, *Article)) error {
	...
	query.Where(predicate.Article(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(author.ArticleColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.author_article
		if fk == nil {
			return fmt.Errorf(`foreign-key "author_article" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "author_article" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
