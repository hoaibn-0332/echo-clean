...
func (aq *ArticleQuery) QueryAuthor() *AuthorQuery {
	query := (&AuthorClient{config: aq.config}).Query()
...
func (aq *ArticleQuery) WithAuthor(opts ...func(*AuthorQuery)) *ArticleQuery {
	query := (&AuthorClient{config: aq.config}).Query()
...
func (aq *ArticleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Article, error) {
	...
	if query := aq.withAuthor; query != nil {
		if err := aq.loadAuthor(ctx, query, nodes, nil,
			func(n *Article, e *Author) { n.Edges.Author = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
